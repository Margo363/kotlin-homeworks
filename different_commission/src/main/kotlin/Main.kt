...
fun commissionPay (
    cardTipe: String = "VkPay",
    limitInMonthVkPay: Int = 0,
    amount: Int = 1
): Int {
    val commission = amount * 0
    val limitInDayVkPay = 1_500_000 //максимальная сумма перевода в копейках за раз Vk Pay
    val limitInMonthVkPayMax = 4_000_000

    if (amount <= limitInDayVkPay) {
        println("комиссия с перевода составила $commission рублей")
    } else if (limitInMonthVkPay > limitInMonthVkPayMax) {
        println("Внимание! Перевод недоступен, максимальная сумма всех переводов в месяц = 40000 рублей.")
...
fun main() {
    val amount = 1_500_001 //сумма перевода в копейках
    val limitInMonthVkPay = 0
    val cardTipe = "VkPay"

    val result = commissionPay(cardTipe, limitInMonthVkPay, amount)
